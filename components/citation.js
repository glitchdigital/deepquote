...
export default ({url, name, publisher, datePublished, exactMatch, keywords, position}) => (
  <div className="flex mb-5 sm:mb-10 z-20 relative">
    <div style={{width: 100}} className="flex-none hidden sm:block">
      <div style={{height: 50, width: 50}} className={classname("mt-5 m-auto bg-white border-4 border-gray-400 rounded-full text-center pt-2 text-gray-600 text-lg font-semibold", {'border-dotted bg-gray-100': !exactMatch, 'border-gray-400 shadow-lg': exactMatch })}>
        {position}
      </div>
    </div>
    <div className="flex-grow">
      <div className={classname("w-full text-left inline-block bg-white rounded-lg", {'bg-white shadow-xl border': exactMatch})}>
