  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      const quotes = getQuotes(text).map(quote => {
        return { 
          text: quote,
          length: quote.length, // Length of the quote (useful for searching)
          wordCount: quote.split(' ').length, // Number of words in the quote (useful for searching)
          count: 0, // The number of times the quote appears in the article
          sentences: [],
          sentiment: {
            posCount: 0,
            negCount: 0,
